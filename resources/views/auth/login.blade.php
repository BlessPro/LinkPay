<x-guest-layout>
    ...
    <p class="mt-2 text-sm text-slate-500">
        @if($phoneAuthEnabled)
            Sign in with phone + PIN by default, or use OTP / email fallback.
        @else
            Sign in with email. Phone auth is currently disabled.
        @endif
    </p>
    ...
    @if($phoneAuthEnabled)
        <div class="mt-6 inline-flex rounded-full border border-slate-200 bg-white/80 p-1 text-sm">
            <button type="button" class="login-tab rounded-full px-4 py-2 font-semibold text-emerald-700" data-target="login-pin">Phone PIN</button>
            <button type="button" class="login-tab rounded-full px-4 py-2 font-semibold text-slate-500" data-target="login-sms">OTP</button>
            <button type="button" class="login-tab rounded-full px-4 py-2 font-semibold text-slate-500" data-target="login-pin-reset">Reset PIN</button>
            <button type="button" class="login-tab rounded-full px-4 py-2 font-semibold text-slate-500" data-target="login-email">Email</button>
        </div>
    @endif

    <div id="login-pin" class="mt-6 {{ $phoneAuthEnabled ? '' : 'hidden' }}">
        <form class="space-y-4" method="POST" action="{{ route('login.phone.pin') }}">
            ...
            <div>
                ...
                <input id="pin" name="pin" value="{{ old('pin') }}" class="mt-2 w-full rounded-xl border-slate-200 bg-white/80 px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" type="password" placeholder="1234" inputmode="numeric" pattern="[0-9]*" maxlength="4" autocomplete="current-password" />
                <x-input-error :messages="$errors->get('pin')" class="mt-2" />
            </div>

            <div class="flex items-center justify-between text-sm">
                <button type="button" class="text-emerald-600 hover:text-emerald-500" onclick="setActiveTab('login-pin-reset')">Forgot PIN?</button>
            </div>
    ...
    <div id="login-pin-reset" class="mt-6 hidden">
    ...
    <div id="login-email" class="mt-6 {{ $phoneAuthEnabled ? 'hidden' : '' }}">
        <form class="space-y-5" method="POST" action="{{ route('login') }}">
            ...
            <div>
                ...
                <div class="relative mt-2">
                    <input id="password" class="w-full rounded-xl border-slate-200 bg-white/80 px-4 py-3 pr-12 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" type="password" name="password" autocomplete="current-password" placeholder="Your secure password" />
                    <button
                        type="button"
                        id="toggle-password"
    ...
    <script>
        ...
        const panels = {
            ...
            'login-pin-reset': document.getElementById('login-pin-reset'),
        ...
        const pinErrors = @json($errors->has('pin'));
        const otpErrors = @json(session('otp_status') || $errors->has('otp'));
        const pinResetErrors = @json(session('pin_reset_status') || $errors->has('reset_phone_number') || $errors->has('reset_otp') || $errors->has('reset_pin'));
        const emailErrors = @json($errors->has('email') || $errors->has('password'));
        const defaultTab = !phoneAuthEnabled
            ? 'login-email'
            : (pinResetErrors ? 'login-pin-reset' : (pinErrors ? 'login-pin' : (otpErrors ? 'login-sms' : (emailErrors ? 'login-email' : 'login-pin'))));
